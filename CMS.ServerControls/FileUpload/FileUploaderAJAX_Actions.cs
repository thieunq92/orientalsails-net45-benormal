    ...
    public partial class FileUploaderAJAX
    {
        ...
        public void SaveAs(string path)
        {
            ...
            if (File_RenameIfAlreadyExists)
            {
                ...
                int contador = 1;
                if (File.Exists(directory + _fileNameWithoutExtension + extension))
                {
                    while (File.Exists(directory + _fileNameWithoutExtension + extension))
                    {
            ...
            {
                path = "/" + path.Replace(Server.MapPath("/").Replace("\\", "/"), "");
            }
